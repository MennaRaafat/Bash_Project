	...
	3)          
		...
		read col_no
	data=""
number=$(wc -l  ~/Downloads/Bash_Project/"$db_name"/"$table_name" | awk '{print $1}')
#number=$(( $number-1 ))
...
#			if [ $m -eq 0 ];then
                        	exitflag=0
isUnique=`awk -F':' -v id="$col_name" '{if($1==id) print}' ~/Downloads/Bash_Project/"$db_name"/"$table_name" `
		       	if [ $col_name == $var ];
	...
	#		then       
		...
		echo "ID number must be unique and not null"
				echo "Enter data "
				read col_data
			fi
